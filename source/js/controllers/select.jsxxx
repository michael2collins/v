...
uis.controller('uiSelectCtrl',
  ...
  // Bind to keyboard shortcuts <><><><><><> grebett's WORKAROUND <><><><><><><>
  // User may use arrows to select choices: this behaviour is enabled by default
  ctrl.arrows = true;
  ctrl.searchInput.on('keydown', function(e) {
    var key = e.which;
 
     // up and down: user is selecting choices
    if (key === 38 || key === 40)
      ctrl.arrows = true;
     // left and right: user is writing again, active index isn't anymore (-1)
    else if (key === 37 || key === 39) {
      ctrl.activeIndex = -1;
      ctrl.arrows = false;
    }
    // for anyother key execpt enter and tab, deactive the arrows flag
    else if (key !== 13 && key !== 9)
      ctrl.arrows = false;
 
    // if the arrow flag is of and the user has hit enter or tab, accept his input as new entry (except if it's empty)
    if ((key === 13 || key === 9) && !ctrl.arrows && ctrl.search.length > 0) {
      ctrl.selected = ctrl.search;
      // reduce items to 0 and apply it to hide the items list
