...
export class ImpclassController {
    ...
    getColumns() {
        ...
        var Columnslist = [
            ...
            { name: "pgmname", type: 'string' },
            { name: "studentClassStatus", type: 'string' },
            { name: "ranktype", type: 'string' },
            { name: "currentRank", type: 'string' },
            { name: "lastPromoted", type: 'date' },
            { name: "payerName", type: 'string' },
            { name: "payerEmail", type: 'string' },
            { name: "paymenttype", type: 'string' },
            { name: "paymentplan", type: 'string' },
            { name: "paymentAmount", type: 'number' },
            { name: "payOnDayofMonth", type: 'number' },
        ];
    ...
    setGColumns(columns) {
        ...
        for (var i = 0, len = columns.length; i < len; i++) {
            ...
            if (columns[i].name.match(/email/i)) {
                theValidStr = {
            ...
            if (columns[i].name.match(/lastPromoted/i)) {
                theValidStr = {
    ...
    setGridexp1Options() {
        ...
        vm.gridexp1Options = {
            ...
            exporterCsvFilename: 'studentregistrations.csv',
...
//                vm.lookupExtas().then(function(){
 ...
 //               });
    ...
    createBulkStudentRegistrations() {
        var vm = this;
        var path = '../v1/bulkstudentregistrations';
        vm.$log.debug('about createBulkStudentRegistrations ');
        var thedata = {
            Pricesetdate: new Date(),
            selectedStudents: vm.gridimp1Options.data
        };

        return vm.StudentServices.createBulkStudentRegistrations(path, thedata)
            .then(function(data) {
                vm.$log.debug('createBulkStudentRegistrations returned data');
                vm.$log.debug(data);
                vm.thisstudent = data;
                ...
                vm.$log.debug('createBulkStudentRegistrations failure:');
