...
dynamicLayoutModule.factory('FilterService', function () {
  ...
  */
  var checkStatement = function(item, statement){
	...
	console.log('typeofstatement' + typeof(statement));
  ...
  */
  var checkOrGroup = function(item, orGroup){
	  console.log('orGroup' + orGroup);
    for(var j in orGroup){
		if (! (orGroup[j] === undefined)) {
  ...
  return {
    ...
    */
    applyFilters: function (items, filters) {
      ...
      for(var i in items){
		  console.log('filter items for ' + i); console.log( items[i]);
		  if (!(i === "first") && !(i === "contains") && !(i === "page") && !(i === "home") && !(i === "where")) {
			if(checkAndGroup(items[i], filters)){
			  retItems.push(items[i]);          
			}
		  }
      }
	  console.log('retItems'); console.log(retItems)
      return retItems;
    }
  };
});